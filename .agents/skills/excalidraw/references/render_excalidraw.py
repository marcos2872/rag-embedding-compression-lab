...
# Limite seguro abaixo dos 8000px máximos aceitos pela API Claude
MAX_PNG_DIMENSION = 6000


def _resize_if_needed(png_path: Path, max_dimension: int) -> None:
    """Redimensiona o PNG proporcionalmente se qualquer dimensão exceder max_dimension.

    Evita que imagens grandes quebrem a API Claude (limite: 8000px por dimensão).
    """
    ...
    print(
        f"INFO: PNG redimensionado de {w}×{h} → {new_size[0]}×{new_size[1]} (limite: {max_dimension}px)"
    )
...
def render(
    ...
    try:
        ...
        print(
            "Run: cd .agents/skills/excalidraw/references && uv sync && uv run playwright install chromium",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print(
                    "Run: cd .agents/skills/excalidraw/references && uv run playwright install chromium",
                    file=sys.stderr,
                )
                sys.exit(1)
            raise
    ...
    _resize_if_needed(output_path, MAX_PNG_DIMENSION)
    return output_path
